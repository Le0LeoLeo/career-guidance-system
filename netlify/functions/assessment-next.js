// netlify/functions/assessment-next.js
...
async function supabaseSelectState(userId) {
  const SUPABASE_URL = process.env.SUPABASE_URL;
  const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;
...
async function supabaseUpsertState(payload) {
  const SUPABASE_URL = process.env.SUPABASE_URL;
  const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;
...
exports.handler = async (event) => {
  try {
    ...
    let state = await supabaseSelectState(userId);
    if (!state) {
    ...
    if (state.current_stage === "done") {
      return json(200, { ok: true, done: true, state: { ...state, unconverged } });
    }
    ...
    const q = await callWenxinFlashGenerateQuestion(aiInput);

    if (!q || typeof q !== "object" || !Array.isArray(q.options) || q.options.length !== 4) {
      return json(500, { error: "AI question invalid" });
    }
    ...
    if (!question.prompt || question.options.some((o) => !o.text)) {
      return json(500, { error: "AI returned empty prompt/options" });
    }
    ...
    state = await supabaseUpsertState({ user_id: userId, question_history: history.slice(-60) });
